...
function simulateSynthesize(voice, provider) {
  ...
  const isGeminiVoice = ['Charon', 'Aoede', 'Puck', 'Kore', 'Fenrir', 'Orus', 'Zephyr', 'Sulafat'].includes(voice);
  const isOpenAIVoice = ['alloy', 'ash', 'ballad', 'coral', 'echo', 'fable', 'nova', 'onyx', 'sage', 'shimmer', 'verse'].includes(voice);
...
async function runTests() {
  ...
  if (errorMsg.includes('404')) {
    console.log(`Fallback would trigger for provider google-genai to: ${getDefaultVoiceForProvider('google-genai')}`);
  }
}
