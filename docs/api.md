...
| `POST /ai/media/character-photo`         | Accepts a JPEG data URL and stores it as a versioned immutable object under `{authorId}/characters/{characterId}/` with long-lived cache headers; returns public URL + bucket-relative path.                                                                                                                                 |
| `DELETE /ai/media/character-photo`       | Deletes a character photo at the provided GCS path under `{authorId}/characters/`; legacy `characters/{authorId}/...` paths remain accepted during migration.                                                                                                                                                                |
