...
```powershell
pwsh -NoProfile -Command "npm run format; npm run lint; npm run typecheck; npm run build; npm test -- --runInBand"
pwsh -NoProfile -Command "npm run deploy"   # immutable Cloud Run container/revision
```

`npm run deploy` wraps `scripts/deploy.ps1`: requires a clean commit, repeats the quality gates, builds and pushes the container, deploys Cloud Run, then verifies the deployed SHA and 100% traffic. `npm run deploy:fast` skips only the local quality gates. Workflow YAML is deployed separately when it changes; the Cloud Build policy deliberately forbids workflow deployment steps.
...
| Env vars          | `NODE_ENV=production`, `PORT=8080`, `GOOGLE_CLOUD_PROJECT_ID`, `GOOGLE_CLOUD_REGION`, `TEXT_PROVIDER=google-genai`, `IMAGE_PROVIDER=google-genai`, `GOOGLE_GENAI_MODEL=gemini-3.6-flash`, `GOOGLE_GENAI_IMAGE_MODEL=gemini-3.1-flash-image`, `IMAGE_ANALYZER_PROVIDER` (optional; falls back to `IMAGE_PROVIDER`), `STORY_GENERATION_WORKFLOW_API_KEY`, etc. |
