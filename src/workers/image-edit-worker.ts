...
Return JSON with fields: subjects (array of key entities), setting (concise environment), composition (framing/angle), palette (dominant colors), mood (few adjectives), key_details (salient props / clothing), lighting (brief), style_notes (artistic style hints).`;
    ...
    logger.warn('Base image description failed; continuing without', {
 ...
 */
export async function processImageEditJob(
  jobId: string,
  ...
  try {
    ...
    switch (imageType) {
      case 'cover':
        result = await processFrontCoverEdit(
          story,
          imageUrl,
          userRequest,
          graphicalStyle,
        ...
        break;
      case 'backcover':
        result = await processBackCoverEdit(
          story,
          imageUrl,
          userRequest,
          graphicalStyle,
      ...
      case 'chapter':
        ...
        result = await processChapterImageEdit(
          story,
          imageUrl,
          userRequest,
          chapterNumber,
...
async function processFrontCoverEdit(
  ...
  try {
    ...
    throw new Error(
      `Failed to download base image: ${downloadError instanceof Error ? downloadError.message : String(downloadError)}`,
    );
  }
  ...
  if (convertToStyle) {
    try {
      baseImageDescription = await describeBaseImage(
        baseImageBuffer,
        guessMimeType(baseImageFilename),
        'front_cover',
        aiContext,
      );
    } catch {
  ...
  try {
    ...
    if (canAttemptEdit) {
      try {
        ...
        if (!imageService.edit) {
          logger.error('Image service reported edit capability but method missing at runtime', {
            storyId: story.storyId,
        ...
        if (/does not support editing|not support editing/i.test(msg)) {
          logger.warn('Edit unsupported; falling back to generate() with reference image', {
            storyId: story.storyId,
            imageType: 'front_cover',
          });
          if (!imageService.generate)
            throw new Error('Image generation not supported by current AI provider');
      ...
      if (!imageService.generate)
        throw new Error('Image generation not supported by current AI provider');
      logger.info('Provider lacks edit(); using generate() with reference image to simulate edit', {
        storyId: story.storyId,
        imageType: 'front_cover',
      });
      const augmentedPrompt = `${imagePrompt}\n\nThe provided reference image MUST anchor subject identity, composition and core visual elements. Only apply style adaptation and the explicitly requested changes.${baseImageDescription ? `\n\nREFERENCE_IMAGE_STRUCTURED_DESCRIPTION:\n${baseImageDescription}` : ''}`;
    ...
    throw new Error(
      `AI image editing failed: ${editError instanceof Error ? editError.message : String(editError)}`,
    );
  }
...
async function processBackCoverEdit(
  ...
  const newBackcoverFilename = extractFilenameFromUri(
    generateNextVersionFilename(backcoverVersionBaseUri),
  );
...
async function processChapterImageEdit(
  ...
  try {
    ...
    throw new Error(
      `Failed to download base chapter image: ${downloadError instanceof Error ? downloadError.message : String(downloadError)}`,
    );
  }
  ...
  const newImageFilename = extractFilenameFromUri(
    generateNextVersionFilename(chapterVersionBaseUri),
  );
  ...
  try {
    ...
    throw new Error(
      `AI chapter image editing failed: ${editError instanceof Error ? editError.message : String(editError)}`,
    );
  }
...
function buildUserRequestWithReference(userRequest?: string, userImageUri?: string): string {
  ...
  const referenceNote =
    'Use the separately provided user reference image as stylistic and compositional inspiration while preserving key identifiable elements of the original story image. Do not copy verbatim; adapt coherently to existing story context.';
