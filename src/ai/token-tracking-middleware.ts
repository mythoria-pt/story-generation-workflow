...
export class TextGenerationMiddleware implements ITextGenerationService {
  ...
  private determineModel(options?: TextGenerationOptions): string {
    ...
    if (provider === 'openai') {
      return process.env.OPENAI_BASE_MODEL || process.env.OPENAI_TEXT_MODEL || 'gpt-5.5';
    } else if (provider === 'google-genai') {
      return process.env.GOOGLE_GENAI_MODEL || 'gemini-3.6-flash';
    }
...
export class ImageGenerationMiddleware implements IImageGenerationService {
  ...
  private determineModel(options?: ImageGenerationOptions): string {
    ...
    if (provider === 'openai') {
      return (
        process.env.OPENAI_IMAGE_TOOL_MODEL || process.env.OPENAI_BASE_MODEL || 'gpt-image-2.5-flare'
      );
    } else if (provider === 'google-genai') {
      return process.env.GOOGLE_GENAI_IMAGE_MODEL || 'gemini-3.1-flash-image';
    }
