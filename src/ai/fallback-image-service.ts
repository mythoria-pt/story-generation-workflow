  ...
  async generate(prompt: string, options?: ImageGenerationOptions): Promise<Buffer> {
    try {
      ...
      try {
        ...
        const fallbackIsSafety = fallbackErr instanceof ImageGenerationBlockedError || (fallbackErr instanceof Error && /PROHIBITED_CONTENT/.test(fallbackErr.message));
        if (!fallbackIsSafety) {
          ...
          try {
            ...
            (fallbackErr as any).originalSafetyMessage = err instanceof Error ? err.message : String(err);
          } catch { /* ignore */ }
        ...
        try {
          ...
          (err as any).fallbackError = fallbackErr instanceof Error ? fallbackErr.message : String(fallbackErr);
        } catch { /* ignore */ }
